...
function Marksheet() {
  ...
  const getHeaders = () => {
    ...
    if (token) {
      ...
      // Also try without Bearer prefix in case API expects just the token
      headers["token"] = token
    }
  ...
  const fetchStudentReport = async (studentId: string) => {
    try {
      ...
      alert(`Error fetching student report: ${error.message}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const generatePDF = async (reportCard: ReportCard) => {
    try {
      ...
      const pdfData = {
        studentName: reportCard.studentInfo.name,
        rollNo: reportCard.studentInfo.studentCode,
        studentClass: reportCard.studentInfo.cls,
        academicYear: "2024-2025",
        date: reportCard.examDate,
        result: `Grade: ${reportCard.grade}`,
        remarks: `Average: ${reportCard.average}% - ${reportCard.grade} Grade`,
        subjects: reportCard.subjects.map((subject) => ({
          name: subject.subject,
          q: subject.marksObtained,
          h: subject.maxMarks,
          f: subject.marksObtained,
        })),
      }
      ...
      a.href = url
      a.download = `${reportCard.studentInfo.name}_${reportCard.examType}_Marksheet.pdf`
      ...
      alert("Error generating PDF. Please check your token and try again.")
    } finally {
      setLoading(false)
    }
  }
  ...
  const filteredStudents = students.filter(
    (student) =>
      ...
      student.cls.toLowerCase().includes(searchTerm.toLowerCase()),
  )
    ...
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      {!token && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4 text-gray-800">API Authentication</h2>
          <div className="flex gap-4">
            ...
            <button
              onClick={() => {
                const inputToken = (document.querySelector('input[type="password"]') as HTMLInputElement)?.value
                if (inputToken) {
          ...
          <p className="text-sm text-gray-600 mt-2">Token will be saved in localStorage for future sessions</p>
        </div>
      )}
                    ...
                    {filteredStudents.map((student) => (
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-600">
                          Class: {student.cls} | Roll: {student.studentCode}
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <div className="flex">
                        ...
                        <span className="text-gray-900">{selectedStudent.studentCode}</span>
                      </div>
                      <div className="flex">
                        <span className="font-semibold text-gray-700 w-32">Class:</span>
                        <span className="text-gray-900">{selectedStudent.cls}</span>
                      </div>
                      <div className="flex">
                      ...
                      <div className="flex">
                        ...
                        <span className="text-gray-900">{selectedStudent.dob}</span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="flex">
                        <span className="font-semibold text-gray-700 w-32">Contact:</span>
                        <span className="text-gray-900">{selectedStudent.contact}</span>
                      </div>
                      <div className="flex">
                      ...
                      <div className="flex">
                        ...
                        <span className="text-gray-900">{selectedStudent.familyDetails?.stdo_FatherName}</span>
                      </div>
                      <div className="flex">
                        <span className="font-semibold text-gray-700 w-32">Mother:</span>
                        <span className="text-gray-900">{selectedStudent.familyDetails?.stdo_MotherName}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-md p-6">
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500 text-lg">No report cards found for this student</p>
                    </div>
                  ) : (
                        ...
                        >
                          <div className="bg-gray-50 px-6 py-4 flex justify-between items-center">
                            <div>
                              ...
                              <p className="text-gray-600">Date: {report.examDate}</p>
                            </div>
                            <div className="text-right">
                              ...
                              <p className="text-sm text-gray-600 mt-1">Average: {report.average}%</p>
                            </div>
                          </div>
                          <div className="p-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                              {report.subjects.map((subject, index) => (
                                <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                  <div className="font-semibold text-gray-900 mb-1">{subject.subject}</div>
                            ...
                            <div className="border-t border-gray-200 pt-4 flex justify-between items-center">
                              <div className="flex items-center gap-6">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="font-semibold text-gray-900">Total: {report.totalMarks}</span>
                                </div>
                                <div className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm font-semibold">
                                  {((report.totalMarks / (report.subjects.length * 100)) * 100).toFixed(1)}%
                                </div>
                              </div>
                              <button
                                onClick={() => generatePDF(report)}
                                disabled={loading}
                                className="flex items-center gap-2 btn button text-white px-6 py-2 rounded-lg  disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
                              >
